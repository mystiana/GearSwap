...
function pet_midcast(spell)
    ...
    if (spell.type == 'BloodPactRage' or spell.type == 'BloodPactWard') then
        ...
        elseif bp_magical:contains(spell.name) then
			...
			elseif pet.tp < 1500 then
				equip(sets.avatar.magical)
			else 
				equip(sets.avatar.magical.HighTP)
            end
...
function pet_change(pet,gain)
	if gain then
		windower.chat.input('//htb summon ' .. pet.name);
		windower.add_to_chat(8,'----- Avatar set to '..mode..' mode! -----')
	else 
		windower.chat.input('//htb summon');
	end
    idle(pet)
	windower.chat.input('//htb reload');
end
